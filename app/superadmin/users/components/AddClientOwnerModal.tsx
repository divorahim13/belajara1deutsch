...
import { createClientOwnerAccount } from '../actions';
...
export default function AddClientOwnerModal({ companies }: { companies: Company[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await createClientOwnerAccount(formData);
  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-lg font-medium hover:bg-indigo-700 transition-colors shadow-sm text-sm"
      >
        <UserPlus size={18} />
        Add Client Owner
      </button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">Add Client Owner</h2>
          <button 
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Provide this password to the client owner.</p>
            </div>
