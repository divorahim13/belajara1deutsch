...
const questions: Question[] = [
  {
    id: 1,
    segments: [
      { type: 'text', val: '1. Das letzte Spiel war eine Katastrophe. Die ' },
      { type: 'input', id: 'q1_1', ans: 'Mannschaft', width: 'w-32' },
      { type: 'text', val: ' hat sehr schlecht gespielt. Der Torwart war nicht fit, ' },
      { type: 'input', id: 'q1_2', ans: 'deshalb', width: 'w-24' },
      { type: 'text', val: ' haben wir drei Tore bekommen. ' },
      { type: 'input', id: 'q1_3', ans: 'Trotzdem', width: 'w-24' },
      { type: 'text', val: ' bleibe ich ein Fan.' }
    ]
  },
  {
    id: 2,
    segments: [
      { type: 'text', val: '2. Sie macht jeden Tag Sport, ' },
      { type: 'input', id: 'q2_1', ans: 'deshalb', width: 'w-24' },
      { type: 'text', val: ' ist sie sehr fit. Sie ist heute ein bisschen müde. ' },
      { type: 'input', id: 'q2_2', ans: 'Trotzdem', width: 'w-24' },
      { type: 'text', val: ' geht sie joggen.' }
    ]
  },
  {
    id: 3,
    segments: [
      { type: 'text', val: '3. Der Weg auf den Berg ist sehr steil und gefährlich. ' },
      { type: 'input', id: 'q3_1', ans: 'Trotzdem', width: 'w-24' },
      { type: 'text', val: ' wandern viele Menschen dort. Ich habe ein bisschen Angst, ' },
      { type: 'input', id: 'q3_2', ans: 'deshalb', width: 'w-24' },
      { type: 'text', val: ' bleibe ich lieber unten.' }
    ]
  },
  {
    id: 4,
    segments: [
      { type: 'text', val: '4. Wir haben am Wochenende keine Zeit, ' },
      { type: 'input', id: 'q4_1', ans: 'deshalb', width: 'w-24' },
      { type: 'text', val: ' können wir nicht klettern gehen. ' },
      { type: 'input', id: 'q4_2', ans: 'Trotzdem', width: 'w-24' },
      { type: 'text', val: ' können wir uns auf einen Kaffee treffen.' }
    ]
  },
  {
    id: 5,
    segments: [
      { type: 'text', val: '5. Mein Team hat gestern verloren. ' },
      { type: 'input', id: 'q5_1', ans: 'Trotzdem', width: 'w-24' },
      { type: 'text', val: ' war das Spiel spannend. Ich spiele selbst nicht so gut Fußball, ' },
      { type: 'input', id: 'q5_2', ans: 'deshalb', width: 'w-24' },
      { type: 'text', val: ' schaue ich lieber im Stadion zu.' }
    ]
  }
];

export default function UebungInteraktiv9() {
  ...
  const handleEvaluateSchreiben = async () => {
    ...
    try {
      const res = await fetch('/api/evaluateSchreiben', {
        ...
        body: JSON.stringify({ 
          text: schreibenText,
          topic: "Schreibe einen Kommentar auf einer Fanseite über das letzte Spiel deines Lieblingsteams. Verwende die Wörter 'deshalb' und 'trotzdem'."
        })
      });
  ...
  return (
        ...
        <div className="clay-card p-8 bg-emerald-50 border-emerald-200 border-b-8">
          ...
          <div className="space-y-6 text-lg font-medium text-slate-800">
            {questions.map(q => {
              ...
              return (
                <div key={q.id} className="flex flex-col gap-2 bg-white p-5 rounded-xl border-2 border-sky-100 shadow-sm">
                  <div className="flex flex-wrap items-center gap-y-3 gap-x-1.5 leading-10">
                    {q.segments.map((seg, idx) => {
                      ...
                      if (status === 'checked') {
                        ...
                        inputClass += 'border-slate-300 bg-slate-50 focus:border-sky-400 focus:bg-sky-50';
                      }
      ...
      {activeTab === 2 && (
        <div className="clay-card p-8 bg-sky-50 border-sky-200 border-b-8">
          ...
          <div className="bg-white p-5 rounded-xl border-2 border-indigo-100 mb-6">
            <h4 className="font-bold text-slate-800 mb-2">Soal:</h4>
            <div className="text-slate-600 italic border-l-4 border-sky-400 pl-3">
              Schreibe einen Kommentar auf einer Fanseite über das letzte Spiel deines Lieblingsteams (30–40 Wörter):<br/><br/>
              1. Nenne das Team und erzähle, ob sie gewonnen oder verloren haben.<br/>
              2. Beschreibe, wie das Spiel war.<br/>
              3. Verwende die Wörter "deshalb" und "trotzdem" in deinem Kommentar.
            </div>
          </div>
          ...
          <textarea
            ...
            placeholder="Ich bin ein großer Fan von FC Bayern. Das letzte Spiel war..."
            className="w-full h-48 p-4 rounded-xl border-2 border-slate-300 focus:border-indigo-500 outline-none resize-none mb-4 font-medium text-slate-800"
          />

          <button 
            onClick={handleEvaluateSchreiben}
            disabled={isEvaluating || !schreibenText.trim() || !!aiFeedback}
            className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-400 text-white font-black py-4 rounded-2xl shadow-lg border-b-4 border-indigo-800 active:border-b-0 active:translate-y-1 transition-all text-xl flex items-center justify-center gap-2"
          >
