  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-8">
      <div className="flex gap-4 border-b-2 border-slate-300 pb-4">
        ...
        <button onClick={() => { setActiveTab(1); setShowResult(false); }} className={`px-6 py-2 rounded-xl font-bold ${activeTab === 1 ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-700'}`}>mit & ohne</button>
        <button onClick={() => { setActiveTab(2); setShowResult(false); }} className={`px-6 py-2 rounded-xl font-bold ${activeTab === 2 ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-700'}`}>Konjunktiv II</button>
      </div>
